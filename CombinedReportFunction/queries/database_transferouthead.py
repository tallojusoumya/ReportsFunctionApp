...
LEFT JOIN
    database_transferouthead t_out
    ON t_out.branch_id = mn.branch_id
...
HIGH_PURCHASE_QTY_QUERY = """
    SELECT 
    transferouthead.id,
    transferouthead.entry_number,
    transferouthead.entry_date,
    transferouthead.branch_id,
    b.name AS branch_name,
    b.domain,
...
JOIN
    database_transferouthead transferouthead
        ON transferouthead.branch_id = ph.branch_id
       AND TO_DATE(transferouthead.entry_date, 'YYYY-MM-DD') = TO_DATE(ph.entry_date, 'YYYY-MM-DD')
JOIN
    database_branch b
        ON transferouthead.branch_id = b.id
WHERE 
...
TABLES = {
    "Database_transferouthead": QUERIES
}