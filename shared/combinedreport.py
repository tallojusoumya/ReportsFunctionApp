...
def upload_to_slack(file_path, channel_id):
    try:
        response = client.files_upload_v2(
            channel=channel_id,
            initial_comment="📊 New report generated and uploaded!",
            file=file_path,
...
def generate_report():
    try:
        # Step 1: Create empty Excel with sheets
        with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            for table_name in tables.keys():
                pd.DataFrame().to_excel(writer, sheet_name=table_name, index=False)

        # Step 2: Load workbook and fill with data
        workbook = load_workbook(excel_path)
        ist = pytz.timezone("Asia/Kolkata")
        report_time = datetime.now(ist).strftime("%Y-%m-%d %H:%M:%S %Z")

        for table_name, query_group in tables.items():
            sheet = workbook[table_name]
            row_pos = 1
            timestamp_cell = sheet.cell(row=row_pos, column=1, value=f"Report generated on: {report_time}")
            timestamp_cell.font = Font(bold=True)
            row_pos += 2

            for report_title, sql in query_group.items():
                title_cell = sheet.cell(row=row_pos, column=1, value=f"🔹 {report_title}")
                title_cell.font = Font(bold=True)
                row_pos += 1
                df = pd.read_sql_query(sql, engine)

                if df.empty:
                    msg_cell = sheet.cell(row=row_pos, column=1, value="No records found")
                    msg_cell.font = Font(bold=True, color="000000")
                    row_pos += 3
                else:
                    for r in dataframe_to_rows(df, index=False, header=True):
                        for c_idx, val in enumerate(r, start=1):
                            sheet.cell(row=row_pos, column=c_idx, value=val)
                        row_pos += 1
                    row_pos += 3

        workbook.save(excel_path)

        # ✅ Upload to Slack here (inside try)
        upload_to_slack(excel_path, SLACK_CHANNEL_ID)
